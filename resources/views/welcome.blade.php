...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased {{-- dark:bg-black dark:text-white/50 --}}">
        <div class="relative {{-- min-h-screen --}} flex flex-col items-center justify-center selection:bg-[#19196c] selection:text-white">
            <div class="relative w-full max-w-2xl px-6 lg:max-w-7xl">
                        ...
                        <template x-for="suggestion in suggestions" :key="suggestion.id">
                            <template x-if="suggestion">
                                <li class="suggestion-item" :class="suggestion.isVoted ? 'voted' : ''">
                                    <button class="suggestion-vote" x-on:click="!suggestion.isVoted && vote(suggestion.id)" x-data="voteForSuggestion()">
                ...
                <footer class="py-4 text-center text-sm text-white {{-- dark:text-white/70 --}}">
                    Developed by <a href="https://github.com/RodrigoPauletti" target="_blank" class="underline">Rodrigo Pauletti</a>
                </footer>
            </div>
        </div>
